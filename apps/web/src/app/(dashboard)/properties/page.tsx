...
import type { Property, PagedProperties } from "@/lib/types";
...
export default function PropertiesPage() {
  ...
  const loadProperties = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch<PagedProperties>(
    ...
    <div>
        ...
        <div>
          <p className="text-sm text-slate-500 mb-4">
            {state.total > state.items.length
              ? `Showing ${state.items.length} of ${state.total} properties`
              : `${state.total} ${state.total === 1 ? "property" : "properties"}`}
          </p>
          <div className="flex flex-col gap-3">
            {state.items.map((property) => (
      ...
      {state.status === "empty" && (
        <div className="text-center py-16">
          <p className="text-base text-slate-500 mb-2">No properties yet.</p>
